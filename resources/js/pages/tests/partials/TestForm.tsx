...
export default function TestForm({ initialData, submitUrl, method = 'post' }: Props) {
    const { data, setData, post, put, processing, errors } = useForm({
        ...
        report_delivery_time: initialData?.report_delivery_time ?? '',
    ...
    return (
        <form onSubmit={submit} className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
            <div className="grid gap-6 p-6 md:grid-cols-2 md:p-8">
                ...
                <div>
                    <label className="mb-1.5 block text-sm font-medium text-gray-700">Short Name</label>
                    <input
                        type="text"
                        ...
                        placeholder="CBC"
                    />
                    {errors.short_name ? <p className="mt-1 text-xs text-red-500">{errors.short_name}</p> : null}
                </div>
                ...
                <div>
                    <label className="mb-1.5 block text-sm font-medium text-gray-700">Sample Type</label>
                    <input
                        type="text"
                        value={data.sample_type}
                        onChange={(event) => setData('sample_type', event.target.value)}
                        className={inputClasses}
                        placeholder="Blood"
                    />
                    {errors.sample_type ? <p className="mt-1 text-xs text-red-500">{errors.sample_type}</p> : null}
                </div>

                <div>
                    <label className="mb-1.5 block text-sm font-medium text-gray-700">Report Delivery Time (hours)</label>
                    <input
                        type="number"
                        min="0"
                        value={data.report_delivery_time}
                        onChange={(event) => setData('report_delivery_time', event.target.value)}
                        className={inputClasses}
                        placeholder="24"
                    />
                    {errors.report_delivery_time ? <p className="mt-1 text-xs text-red-500">{errors.report_delivery_time}</p> : null}
                </div>
            ...
            <div className="flex items-center justify-between border-t border-gray-100 px-6 py-5 md:px-8">
                ...
                >
                    ...
                    {method === 'put' ? 'Update Test' : 'Create Test'}
                </button>
            </div>
        </form>
    );
